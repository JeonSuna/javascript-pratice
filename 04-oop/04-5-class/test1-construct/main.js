...
//클래스(class예약어로 선언되는 객체 생성 모형) 내에는 3개의 구성요소가 추가될 수 있다
//변수 함수 생성자
class User{
    name = '홍길동'
    sayHello() {
...
//위처럼 개발자가 명시적으로 클래스내에 생성자를 추가하지 않았다면, 아래에 선언한 것과 동일하게 자동으로
//defalut constructor기 추가된다

class User2{
    name = '홍길동'
    //constructor예약어로 선언되는 함수
    //매개변수를 가지지 않는 생성자를 default constructor 
    //생성자 함수는 어디선가 new로 객체 생성 시점에 객체 생성 역할을 하는것이 중요함으로
    //아래처럼 {}로직을 가지지 않는 생성자도 그 자체로 의미가 있다
    //원한다며 {}에 로직을 추가해서 객체 생성과 동시에 개발자의 로직이 실행되게 할 수 있다.
    constructor() { }
...
let obj2 = new User2()
obj2.sayHello()//hello, 홀길동

//자동으로 추가되는 생성자는 매개변수 없는 생성자
//객체 생성하면서 데이터를 전달 받아서 생성되어야 한다면, 개발자가 명시적으로 생성자 선언
