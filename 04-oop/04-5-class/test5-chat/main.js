...
class Emoji { //매게변수필요
    ...
    //클래스의 statuc과 생성자 함수의 prototype은 다르다
    //객체 생성의 메모리에 매번 할당되지 않는다는 공통점만 있지 이용 및 목적이 다르다
    //prototype은 객체로 이용한다
    //static은 객체로 이용이 불가능하다 => 클래스명으로 이용한다 

    // 클래스내에 함수를 선언할 때, 선언식으로 추가하거나 표현식으로 추가하거나 
    // 선언식으로 추가하면 내부적으로는 prototype에 추가 
    // 표현식으로 추가하면 객체 메모리에 추가된다 
    add(memberId) {
        this.count++
        this.members.push(memberId)
    }
